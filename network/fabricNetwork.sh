...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of pharma-net and is unsupported. Either move to a later version of Fabric or checkout an earlier version of pharma-net."
      exit 1
    fi

    echo "$DOCKER_IMAGE_VERSION" | grep -q "$UNSUPPORTED_VERSION"
    if [ $? -eq 0 ]; then
      echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of pharma-net and is unsupported. Either move to a later version of Fabric or checkout an earlier version of pharma-net."
      exit 1
    fi
  done
}
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/manufacturer.pharma-net.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/MANUFACTURER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
  cd crypto-config/peerOrganizations/distributor.pharma-net.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/DISTRIBUTOR_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
  cd crypto-config/peerOrganizations/retailer.pharma-net.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/RETAILER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
  cd crypto-config/peerOrganizations/consumer.pharma-net.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/CONSUMER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
  cd crypto-config/peerOrganizations/transporter.pharma-net.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/TRANSPORTER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile OrdererGenesis -channelID sys-channel -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
